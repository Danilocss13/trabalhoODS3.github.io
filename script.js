...
// ============================================================
function analisar() {
  if (alimentosSelecionados.length > 0) { mostrarTabela(alimentosSelecionados); return; }

  const input = document.getElementById("alimentos").value;
  ...
  if (resultados.length === 0) { alert("Nenhum alimento encontrado. Tente outro termo."); return; }
...
function mostrarTabela(dados) {
  ...
  dados.forEach(item => {
    html += `<tr>
      ...
      <td>${item.energy_kcal?.toFixed(1) || "N/A"}</td>
      <td>${item.protein_g?.toFixed(1) || "N/A"}</td>
      <td>${item.carbohydrate_g?.toFixed(1) || "N/A"}</td>
      <td>${item.lipid_g?.toFixed(1) || "N/A"}</td>
      <td>${item.fiber_g?.toFixed(1) || "N/A"}</td>
    </tr>`;
  });
...
async function init() {
  ...
  document.querySelectorAll("button").forEach(btn => {
    if (btn.textContent.trim() === "Começar Agora") {
